...
{
    //cout << "Main program, only doing phase test for now" << endl;
...
bool  storePhase(fstream &file)
{
  ...
  //Loops through delay/event data, skipping text and getting hex values
  //Start the TDC data storage:
  //cout << "starting to find phase test" << endl;
  while(data != ("phase_test"))
    //while(data != ("delay=00"))
  {
      //cout << data << endl;
      if(!readline(file,linenumber,data,previousline)) {
  ...
  //cout << "Found phase_test, data = " << data << endl;
  while(data != ("next"))
    {
      if(!readline(file,linenumber,data,previousline)) {
        //cout << "ended 1a"<< endl;
  ...
  for(int delay=0; delay<100; delay++)
    {
	  ...
	  for(int i=0; i<22; i++)
	      ...
	      istringstream(data) >> hex >> TDCPhaseRead[delay][event][i];
            }
...
int  doPhaseTest(string myfile, string outString) {
  ...
  int PhaseCapID[100][4][22];
  int Phase[100][4][22];
  int PhaseRange[100][4][22];
  int PhaseMant[100][4][22];
  double PhaseCharge[100][4][22];

  double time[100];   //will store manipulated "time" that overlays the 3 separate TDC ramps
  double inPhase[100];  //critical TDC value for given event

  double time_fit[75];
  double inPhase_fit[75];

  //for loop COMPRESSES 3 TDC RAMPS INTO 1 & DOES hEXTOBEAUTIFUL, BUT MUCH SHORTER...
  for(int delay=0; delay<100; delay++) {
    time[delay] = delay/2.;
    if(delay == 50) time[delay] = 0.;
    ...
    for(int event = 0; event < 4; event++) {
      for(int slot=0; slot < 22; slot++) {
        ...
        PhaseCapID[delay][event][slot] = (TDCPhaseRead[delay][event][slot] & 0xC000) >> 14;
        Phase[delay][event][slot] = (TDCPhaseRead[delay][event][slot] & 0x3F00) >> 8;
	    PhaseRange[delay][event][slot] = (TDCPhaseRead[delay][event][slot] & 0x00C0) >> 6;
	    PhaseMant[delay][event][slot] = TDCPhaseRead[delay][event][slot] & 0x003F;
	    PhaseCharge[delay][event][slot] = LookupCharge(PhaseRange[delay][event][slot], PhaseMant[delay][event][slot]);
      }
    }
  }
  ...
  for(int delay=0; delay<100; delay++) {
    ...
    for(int i=3; i<22; i++) {
      ...
      if(Phase[delay][whichEvent][i]==63) continue;

      if(found != true) {
	    inPhase[delay] = Phase[delay][whichEvent][i];
      }
      found = true;
  ...
  ********************/
  ...
  for (int i=0; i<100; i++) {
    ...
    if(!foundInt) {
      if(inPhase[i] == 0 && inPhase[i+1] == 49) {
	foundInt = 1;
	interval = i;
      }
      else if(inPhase[i] == 1 && inPhase[i+1] == 49) {
	foundInt = 1;
	interval = i+1;
	inPhase[i+1] = 0;
      }
  ...
  for (int i=0; i<100; i++) {
    ...
    if(time[i] < (interval/2.)) time[i] += 24.5;
    else if(time[i] > (interval/2. + 24.5)) {
      while(time[i] > (interval/2. + 24.5)) time[i] -= 24.5;
    }
    //time[i] -= interval/4.;
  ...
  //for(int i =0; i<100; i++) {
     //outFile << "time: " <<  time[i] << ", phase: " << inPhase[i] << endl;
  //}

  //for(int i=0; i<75; i++) {
  //  time_fit[i] = time[65+i];
  //  inPhase_fit[i] = inPhase[65+i];
  //}

  gsl_fit_linear(time, 1, inPhase, 1, 100, &c0, &c1, &cov00, &cov01, &cov11, &chisq);

  c1 = -1*c1;

  //outFile << "c1 = " << c1 << ", TDC Bin Width " << 1/c1 << endl;
  if(c1 <= 0) outFile << "195  0000" << endl;
  else if(c1 < 0.016) outFile <<"195  FFFF" << endl;
  else outFile << "195  " << setfill('0') << setw(4) << hex << int(1000/c1) << endl;


  double DNLphase[100];
  double Phasefit[100]; //Gives the tdc value of fitted line at time[i]
  for (int i=0; i<100; i++) {
    Phasefit[i]=(c0 - c1*time[i]);
    //outFile << "Time: "<<time[i]<<"  TDC: "<<inTDC[i]<<"  TDCfit Value: "<<TDCfit[i];
    //DNLtdc[i]=(inTDC[i] - TDCfit[i])/TDCfit[i];
    DNLphase[i]=(inPhase[i] - Phasefit[i]);

    if(DNLphase[i] > 25) DNLphase[i] = 50 - DNLphase[i];
    if(DNLphase[i] < -25) DNLphase[i] = 50 + DNLphase[i];

    //outFile << "  TDC Offset: "<<DNLtdc[i]<<endl;
    if(DNLphase[i] < -10) outFile << "196  0000" << endl;
    else if(DNLphase[i] > 600) outFile << "196  FFFF" << endl;
    else outFile << "196  "<< setfill('0') << setw(4) << hex << int((DNLphase[i]+10)*100)<<endl;
...
double  LookupDAC(int DAC, int whichRange) {
    ...
    //QIE10 test board DAC to fC calibration
